...
def run_batch(input_path: str, out_dir: str, lam: float, tth_min: float, tth_max: float, fwhm: float, eta: float, workers: int):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(process_single_cif, p, out_base, lam, tth_min, tth_max, fwhm, eta) for p in paths]
        for i, future in enumerate(concurrent.futures.as_completed(futures), 1):
            result, error = future.result()
            if result:
                ok += 1
                print(f"[{i}/{len(paths)}] {result}")
            if error:
                fail += 1
                p, tb = error
                print(f"[{i}/{len(paths)}] 失败: {p}")
...
def parse_args(argv=None):
    ...
    ap.add_argument('--workers', type=int, default=4, help='并行进程数（默认 4，根据CPU调整以控制发热）')
...
def main():
    ...
    # 兜底：无 Tk 时走命令行参数
    args = parse_args()
    run_batch(args.input, args.out, args.lam, args.tth[0], args.tth[1], args.fwhm, args.eta, args.workers)
