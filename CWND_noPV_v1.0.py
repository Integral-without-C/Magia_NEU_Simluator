...
for atom in asym_atoms:
    ...
    for pos in equiv_pos:
        all_atoms.append({'pos': pos, 'b': b_dict[atom['label']], 'occ': atom['occ'], 'b_iso': debye_waller.get(atom['label'], 0.5)})
print(all_atoms)
# 波长和2theta范围
...
for tt, I, hkl, m in peaks:
    print(f"2θ = {tt:.2f}°, 强度 = {I:.2f}, hkl = {hkl}, 多重度 = {m}")
...
if peaks:
    ...
    for tt, I in zip(two_thetas, intensities):
        plt.vlines(tt, 0, I, color='b', linewidth=1.2)
    plt.xlabel('2θ (degrees)')
    plt.ylabel('Intensity (arbitrary units)')
    plt.title('Simulated Neutron Powder Diffraction Pattern (Stick Plot)')
    plt.tight_layout()
    plt.show()
else:
    print("无峰在范围内。")