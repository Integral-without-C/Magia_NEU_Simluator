#CIF_reading_v1.2.py
##增强了对 CIF 文件格式的容错处理，尤其是对原子位置行的解析。
"""
cif_reader.py
...
轻量级 CIF 解析器 —— 提取并打印：
 ...
 4) 对称性操作列表
 4) 所有原子的信息（坐标、占有率 occupancy、Biso 或 Uiso、wyckoff位点）
        ...
        if any(k.startswith('_atom_site_') for k in lowkeys) and any(k == '_atom_site_label' for k in lowkeys):
            ...
            if rows:
                ...
                for r in rows:
                    ...
                    # 修复：仅将 _atom_site_type_symbol 作为元素列（或包含 type_symbol 的等价变体），不要误用 _atom_site_adp_type
                    elem_idx = None
                    if '_atom_site_type_symbol' in mapping:
                        elem_idx = mapping['_atom_site_type_symbol']
                    else:
                        for lk, idx in mapping.items():
                            if lk.startswith('_atom_site_') and 'type_symbol' in lk:
                                elem_idx = idx
                                break
                    ...
                    # occupancy / Biso / Uiso
                    atom['occupancy'] = get_by_lk('_atom_site_occupancy')
                    atom['Biso'] = get_by_lk('_atom_site_b_iso_or_equiv') or get_by_lk('_atom_site_b_iso')
                    atom['Uiso'] = get_by_lk('_atom_site_u_iso_or_equiv') or get_by_lk('_atom_site_u_iso')
                    ...
                    atom['multiplicity'] = (int(r[mult_idx]) if (mult_idx is not None and mult_idx < len(r) and str(r[mult_idx]).isdigit()) else None)
...
def get_space_group(data: dict) -> str | None:
    """返回更可靠的空间群字符串（优先 H-M 名称，其次 Hall，再次 IT 号/任意空间群相关字段）。"""
    # 优先候选（常见）
    preferred = [
    ...
    # 其他可能包含 SG 名称的键（作为兜底）
    fallback_contains = [
        'space_group_name', 'hall', 'hermann_mauguin',
    ]
    # IT 号码
    it_number_keys = [
    ...
    lowmap = {k.lower(): k for k in data.keys()}

    # 1) 明确优先
    for k in preferred:
        if k.lower() in lowmap:
            return str(data[lowmap[k.lower()]]).strip().strip("'\"")

    # 2) 任意包含关键信息的键
    for lk, orig in lowmap.items():
        if any(t in lk for t in fallback_contains):
            return str(data[orig]).strip().strip("'\"")

    # 3) IT 号
    for k in it_number_keys:
        if k.lower() in lowmap:
            return str(data[lowmap[k.lower()]]).strip().strip("'\"")

    # 4) 兜底：返回首个以 _space_group / _symmetry_space_group 开头的值
    for kd in data.keys():
        kl = kd.lower()
        if kl.startswith('_space_group') or kl.startswith('_symmetry_space_group'):
            return str(data[kd]).strip().strip("'\"")
    return None
...
def list_cif_files(path: str | Path) -> list[Path]:
    """返回目录（递归）或单文件路径中的所有 CIF 文件列表。"""
    p = Path(path)
    if p.is_file() and p.suffix.lower() == '.cif':
        return [p]
    if p.is_dir():
        return sorted(p.rglob('*.cif'))
    return []
...
def main(path):
    ...
    if p.is_dir():
        files = list_cif_files(p)
        print(f'Found {len(files)} CIF files under: {p}')
        for fp in files:
            try:
                ...
                out = {
                    ...
                    'n_symmetry_ops': len(symmetry_ops),
                }
                print(json.dumps(out, ensure_ascii=False))
            except Exception as e:
                print(json.dumps({'file': str(fp), 'error': str(e)}, ensure_ascii=False))
        return
    ...
    print(json.dumps({k:v for k,v in out.items() if k!='atom_sites'}, indent=2, ensure_ascii=False))
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        if not selected:
            print('Usage: python cif_reader.py <cif-file-or-dir>')
        else:
            main(selected)
    else:
        main(sys.argv[1])
